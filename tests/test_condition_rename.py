...
from tombot.config import CONDITIONS, RETIRED_CONDITIONS, Config
...
def test_edited_multipliers_for_live_grades_survive(tmp_path):
    """Re-seeding on startup must not undo what someone deliberately changed."""
    repo = _repo(tmp_path)
    repo.set_modifier("condition", "EX", 0.91)
    repo.init_db()                       # re-seed: INSERT OR IGNORE, keeps 0.91

    assert repo.get_modifiers()["condition"]["EX"] == pytest.approx(0.91)
