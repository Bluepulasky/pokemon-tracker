...
### 2.19 🔴 TCGdex gives one Cardmarket product to several cards

A scan of all 1,104 cards across the twelve sets found **89 Cardmarket products
claimed by more than one card, covering 199 cards** — 18% of the catalog.

Two distinct shapes:

* **Holo / non-holo pairs (82 groups).** WOTC sets print a rare as both a holo
  and a non-holo with different numbers; Cardmarket sells them as separate
  products, TCGdex reports one. The non-holo inherits the holo's price and runs
  four to six times high. Jungle 32/64, Fossil 30/62, Neo Discovery 35/75.
* **Gym trainer name bleed (7 groups).** `Brock`'s holo product also lands on
  `Brock's Geodude`, `Brock's Onix` and the rest — commons valued as holo rares.
  Same for Erika, Misty, Koga, Sabrina, Giovanni and Lt. Surge.

Verified against pokemontcg.io, which maps one product per card:

| card | TCGdex | pokemontcg.io | error |
|---|---|---|---|
| base2-19 Flareon, non-holo | 46.57 | 10.39 | 4.5x |
| base2-17 Clefable, non-holo | 31.14 | 5.33 | 5.8x |
| base3-18 Ditto, non-holo | 39.19 | 7.48 | 5.2x |

**Decision — refuse a shared product rather than switch source.** Base Set has
zero collisions and is the one place TCGdex earns its keep: 105 cards there
carry genuinely separate products per print run, which is where Shadowless
pricing comes from. And pokemontcg.io is not clean either — it returns 726.67
EUR for Jungle Clefable holo, which is nonsense.

So a quote whose product is claimed by another card is refused, and the card is
priced from the second provider instead. The affected products are listed in
`tombot/data/tcgdex_shared_products.json`, regenerated by
`scripts/tcgdex_scan.py`.

**Not averaged.** TCGdex's 46.57 for Flareon #19 is not an opinion about that
card, it is the correct price of Flareon #3. Averaging it with the real 10.39
yields 28.48, which describes no card that exists, and turns a visible 4.5x
error into a believable one. Sources are combined only once they agree on
identity.

### 2.20 🟠 One number hid the problem, so every quote is kept

`price_cache` holds the single figure a card is valued at. It could not show
that two providers quoting the same market disagreed by 4x, which is exactly
the signal that one of them has the wrong card.

`price_quotes` now records every quote from every provider and market, with a
`trusted` flag and the reason when it is 0. The modal lists them instead of
reducing them, opening itself when something was refused.

pokemontcg.io ships TCGplayer prices in the payload the catalog already fetches
— a genuinely independent market, split by print run, at no extra request. It
is shown for reference and deliberately kept out of the valuation: different
market, different currency, and Tom buys and sells on Cardmarket.
